...
with open("log.csv", "w") as log:
    ...
    for i in range(len(cities)):
        ...
        fig1.suptitle(f"{cities[i]} XGBClassifier")
        fig1.savefig(
            f"plot_zero_part/{cities[i]}XGBClassifier_1.pdf")
        fig2.suptitle(f"{cities[i]} XGBClassifier")
        fig2.savefig(
            f"plot_zero_part/{cities[i]}XGBClassifier_2.pdf")
        fig3.suptitle(f"{cities[i]} XGBClassifier")
        fig3.savefig(
            f"plot_zero_part/{cities[i]}XGBClassifier_3.pdf")
        fig4.suptitle(f"{cities[i]} XGBClassifier")
        fig4.savefig(
            f"plot_zero_part/{cities[i]}XGBClassifier_4.pdf")
        ax1.clear()
        ax2.clear()
        ax3.clear()
        ax4.clear()
        ...
        for j in range(1, 101):
            ...
            precs[j - 1] = prec
            recs[j - 1] = rec
            print(cities[i], "RandomForestClassifier",
                j, acc, score)
        ...
        ax4.set_xlabel("n_estimator")
        ax4.set_ylabel("recall")
        fig1.suptitle(f"{cities[i]} RandomForestClassifier")
        fig1.savefig(
            f"plot_zero_part/{cities[i]}RandomForestClassifier_1.pdf")
        fig2.suptitle(f"{cities[i]} RandomForestClassifier")
        fig2.savefig(
            f"plot_zero_part/{cities[i]}RandomForestClassifier_2.pdf")
        fig3.suptitle(f"{cities[i]} RandomForestClassifier")
        fig3.savefig(
            f"plot_zero_part/{cities[i]}RandomForestClassifier_3.pdf")
        fig4.suptitle(f"{cities[i]} RandomForestClassifier")
        fig4.savefig(
            f"plot_zero_part/{cities[i]}RandomForestClassifier_4.pdf")
        ax1.clear()
        ax2.clear()
        ax3.clear()
        ax4.clear()
        ...
        # matplotlib.pyplot.title(f"{cities[i]} SVC")
        # matplotlib.pyplot.savefig(
        #     f"plot/{cities[i]}SVC.pdf")
        ...
        # print(f"{cities[i]} SVC {test_score}")
        # test_score_svc = test_score
        with open(f"tex_zero_part/{cities[i]}.tex", "w") as fout:
