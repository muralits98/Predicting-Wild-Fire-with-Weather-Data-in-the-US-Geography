...
for i in range(len(cities)):
    ...
    for j in range(1, 101):
        clf = util.EnsembleClassifier(n_ensemble = j,
                                      n_components = 10)
        ...
        precs[j - 1] = prec
        recs[j - 1] = rec
        print(cities.iloc[i, 0], "EnsembleClassifier",
            j, acc, score)
        if (score > max_score):
            par = j
            max_score = score
    ...
    fig1.suptitle(f"{cities.iloc[i, 0]} EnsembleClassifier")
    fig1.savefig(
        f"plot/{cities.iloc[i, 0]}EnsembleClassifier_1.pdf")
    fig2.suptitle(f"{cities.iloc[i, 0]} EnsembleClassifier")
    fig2.savefig(
        f"plot/{cities.iloc[i, 0]}EnsembleClassifier_2.pdf")
    fig3.suptitle(f"{cities.iloc[i, 0]} EnsembleClassifier")
    fig3.savefig(
        f"plot/{cities.iloc[i, 0]}EnsembleClassifier_3.pdf")
    fig4.suptitle(f"{cities.iloc[i, 0]} EnsembleClassifier")
    fig4.savefig(
        f"plot/{cities.iloc[i, 0]}EnsembleClassifier_4.pdf")
    ax1.clear()
    ax2.clear()
    ax3.clear()
    ax4.clear()
